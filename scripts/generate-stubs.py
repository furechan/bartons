"""Generate `python/bartons/plugin.pyi` from the compiled extension.

The plugin is a compiled `.so`, so type checkers cannot see into it — `ty`
reports `Module bartons has no member plugin` for every import. A stub fixes
...
HEADER = '''"""Type stubs for the compiled `bartons.plugin` extension.
...
def main() -> None:
    functions = sorted(
        ...
        if inspect.isbuiltin(obj := getattr(plugin, name))
    )
    if not functions:
        raise SystemExit("generate-stubs: no pyfunctions found — is the plugin built?")
    ...
    exported = {n for n in getattr(plugin, "__all__", []) if not n.startswith("__")}
