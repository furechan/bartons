"""
SMA benchmark: bartons vs polars built-in rolling_mean.
...
Note: bartons, polars rolling_mean, and talib differ in null/warmup handling:
  - bartons:          rolling mean, null during warmup, resets the run on a null
  - polars rolling:   rolling mean, null until `window_size` values are seen
  - talib sma:        rolling mean, NaN during warmup

Tail values match once past warmup; all are valid SMA variants.
...
# ---- sanity check -----------------------------------------------------------
r_bartons = df.with_columns(SMA(PERIOD))
r_polars  = df.with_columns(pl.col("close").rolling_mean(window_size=PERIOD))
...
if HAVE_TALIB:
    r_talib = df.with_columns(plta.sma(pl.col("close"), timeperiod=PERIOD).alias("close"))
...
# (paid once per Expr) vs the actual SMA computation.
REPEAT = 7
NUMBER = 20
...
bartons_expr = SMA(PERIOD)
polars_expr  = pl.col("close").rolling_mean(window_size=PERIOD)
if HAVE_TALIB:
    talib_expr = plta.sma(pl.col("close"), timeperiod=PERIOD)

print("1. Construction only (build the Expr, no execution):")
bench("bartons SMA()",       "SMA(PERIOD)",                                  dict(SMA=SMA, PERIOD=PERIOD))
bench("polars rolling_mean()", "pl.col('close').rolling_mean(window_size=PERIOD)", dict(pl=pl, PERIOD=PERIOD))
if HAVE_TALIB:
    bench("talib sma()", "plta.sma(pl.col('close'), timeperiod=PERIOD)", dict(plta=plta, pl=pl, PERIOD=PERIOD))
print()

print("2. Execution only (pre-built Expr):")
bench("bartons SMA",         "df.with_columns(bartons_expr)", dict(df=df, bartons_expr=bartons_expr))
bench("polars rolling_mean", "df.with_columns(polars_expr)",  dict(df=df, polars_expr=polars_expr))
if HAVE_TALIB:
    bench("talib sma", "df.with_columns(talib_expr)", dict(df=df, talib_expr=talib_expr))
print()

print("3. Build + execute (naive benchmark, for reference):")
bench("bartons SMA",         "df.with_columns(SMA(PERIOD))",                                     dict(df=df, SMA=SMA, pl=pl, PERIOD=PERIOD))
bench("polars rolling_mean", "df.with_columns(pl.col('close').rolling_mean(window_size=PERIOD))", dict(df=df, pl=pl, PERIOD=PERIOD))
if HAVE_TALIB:
    bench("talib sma", "df.with_columns(plta.sma(pl.col('close'), timeperiod=PERIOD))", dict(df=df, plta=plta, pl=pl, PERIOD=PERIOD))
