...
`uv run ty check` is **clean** and should stay that way, with no exclusions —
`python/bartons/plugin.pyi` (generated — see below) covers the compiled
extension, which a checker cannot otherwise see into, and that is the whole of
it. The runtime-registered `.bt` namespace was retired partly because it could
not be typed at all; see [docs/namespace-legacy.md](docs/namespace-legacy.md).
...
- [docs/benchmark-builder-vs-collect.md](docs/benchmark-builder-vs-collect.md) — recorded kernel micro-benchmark (2026-08-17): the `Filter` abstraction is free, `append_option` costs ~1.6× — read before touching the append in `run_unary`/`run_ternary`
