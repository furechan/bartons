...
**Rust side — [rust/Cargo.toml](../../rust/Cargo.toml) `[dependencies]`:**
- `pyo3` — version + features (`extension-module`, `abi3-py*`)
- `pyo3-polars` — version + features
- `polars` — version
- `polars-arrow` — version

**Rust side — resolved:** read the actual resolved `polars` crate version from `rust/Cargo.lock` (`grep -A1 '^name = "polars"' rust/Cargo.lock`). The `Cargo.toml` range is the *request*; the lockfile is what was *built*.
