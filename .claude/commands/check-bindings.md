...
- `[project] dependencies` — **is there an enforced `polars >=x,<y` cap here?** This is the install-time gate the lockstep note argues for; note explicitly if it is missing.
- `[dependency-groups] dev` — the `polars` used for testing (note if unpinned)
...
4. **The install cap.** If `[project].dependencies` has no `polars >=x,<y`, the coupling is *de facto* (works) but not *enforced* (nothing stops a bad polars from being installed). Call this out.
5. **pyo3 abi3 floor vs requires-python** — sanity only.
