...
## FFI version history (polars-rs axis)

The FFI version is two `u16` constants, `MAJOR`/`MINOR`, in the `polars-ffi` crate `src/lib.rs`. `polars-ffi` is versioned in lockstep with polars-rs, so this column doubles as the polars-crate version.

| polars-rs (and `polars-ffi`) | FFI version | Notes |
|---|---|---|
...
- Some polars-rs versions were never shipped in a stable polars-py release (e.g. `0.47.x`, `0.54.1`–`0.54.3`); Python minors skip across them.
...
The plugin side exports the FFI version via `pyo3-polars`. For reference, the export symbol and the polars-rs version each `pyo3-polars` pins:

| pyo3-polars | exports `_polars_plugin_get_version` | polars-rs `req` |
|---|---|---|
...
3. For each polars-py release, download the sdist and read `[workspace.package] version` from the root `Cargo.toml` (→ Rust crate version); cross-check `crates/polars-ffi/src/lib.rs` for the FFI constants. Use `polars-runtime-32` for ≥ `1.34.0`.
